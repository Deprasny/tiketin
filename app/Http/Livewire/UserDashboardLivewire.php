...
class UserDashboardLivewire extends Component
{
    ...
    public function render()
    {
        ...
        if (isset($this->search)) {
            $passangers = Passanger::where('user_id', Auth::user()->id)->where('name', 'like', '%' . $this->search . '%')->paginate(4);
        } else {
            $passangers = Passanger::where('user_id', Auth::user()->id)->orderBy('id', 'DESC')->paginate(4);
        }
        $stations = Station::all();
        $tickets = Ticket::where('user_id', Auth::user()->id)->orderBy('id', 'DESC')->get();
        return view('livewire.user-dashboard-livewire', ['passangers' => $passangers, 'stations' => $stations, 'tickets' => $tickets]);
    }
